import { confirmedModalAtom, registrationPhoneAtom, useConfirmCode } from '@/features'
...
export const ConfirmPhone: FC = () => {
  ...
  const { mutate, isLoading } = useConfirmCode({
    onSuccess: () => {
  ...
  const generateTitleConfirmPhone = () => {
    if (isProfilePrivate) return t('common:Confirm')
    return t('signUp')
  }

  return (
    <div className='flex flex-col desktop:w-[500px] w-full gap-[23px] select-none'>
      <h1 className='croogla-secondary-text'>{generateTitleConfirmPhone()}</h1>
      ...
      <Form<RequestConfirmRegPhone>
        ...
        onSubmit={data => mutate({ ...data, phone: phoneAfterRegistration, isRepeat: isProfilePrivate })}
