...
export const atomWithUrlLocation = <Value>(key: string, initialValue: Value, options: AtomWithHashOptions<Value>) => {
  ...
  function updateQuery(key: string, newValue?: Value | string | number): void {
    const searchParams = getURLParams()
    newValue || typeof newValue === 'number' ? searchParams.set(key, serialize(newValue)) : searchParams.delete(key)
    ...
    Object.assign(
      {
        delayInit: false,
        subscribe: callback => {
          isTestEnv || Router.events.on('routeChangeStart', callback)
          window.addEventListener(eventName, callback)
          return () => {
            isTestEnv || Router.events.off('routeChangeStart', callback)
