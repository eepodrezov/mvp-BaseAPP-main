  ...
  return (
    <OptionalLinkWrapper href={file.path} newTab>
      {isImage ? (
       ...
       bg-border hover:bg-text active:bg-border transition-colors border-none`,
            className
          )}
        >
          <p className='w-[116px] source-text text-black truncate'>{file.name}</p>
          {isLoading ? (
            <Loading data-testid='file-preloader' className='fill-black w-4 h-4 mr-2 animate-spin shrink-0' />
          ) : (
            <CloseButton />
          )}
        </div>
