...
import { act, fireEvent, render, screen, waitFor } from '@/jest/utils'
...
describe('TooltipTests', () => {
  ...
  it('tooltip should be render with click and no render with hover', async () => {
    const component = render(<Default isClickable />)
    const user = userEvent.setup({ delay: null })
    user.hover(component.getByRole('button'))
    await waitFor(() => expect(screen.queryByRole('tooltip')).not.toBeInTheDocument())
    jest.useFakeTimers()
    await user.click(screen.getByRole('button'))
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Test Storybook tooltip' })).toBeInTheDocument())
    fireEvent.blur(component.getByRole('button'))
